...
// fakeLoop is a minimal domain.Loop that records whether it ran and
// returns a fixed result — used to prove BindSkill actually uses
// agent.Loop when set, instead of always constructing its own NativeLoop.
type fakeLoop struct {
	called bool
}

func (f *fakeLoop) Run(_ context.Context, messages []domain.Message, _ []*domain.BoundAction) (*domain.LoopResult, []domain.Message, error) {
	f.called = true
	return &domain.LoopResult{Status: domain.LoopStatusComplete, Output: map[string]any{"url": "https://example.com/pr/2"}}, messages, nil
}

// TestBindSkill_UsesAgentLoopWhenSet is a regression test for a real bug:
// domain.Agent.Loop was documented as pluggable ("nil means use
// NewNativeLoop") but BindSkill's multi-tool path never actually checked
// it, always constructing a NativeLoop from agent.LLMs regardless. agent.LLMs
// is deliberately left empty here — if BindSkill fell back to NativeLoop
// despite agent.Loop being set, Run would fail with "no LLM configured"
// instead of returning fakeLoop's result.
func TestBindSkill_UsesAgentLoopWhenSet(t *testing.T) {
	loop := &fakeLoop{}
	lookupRepo := &domain.BoundAction{
		Spec:   domain.ActionSpec{Name: "lookup_repo"},
		Invoke: func(_ context.Context, _ map[string]any) (any, error) { return "kael", nil },
	...
	agent := &domain.Agent{ID: "agent_dev", Loop: loop, MaxIterations: 5}

	bound := domain.BindSkill(agent, skill, []*domain.BoundAction{lookupRepo, lookupBranch})
	result, err := bound.Invoke(context.Background(), map[string]any{})
	if err != nil {
		t.Fatalf("invoke: %v", err)
	}
	if !loop.called {
		t.Fatal("expected agent.Loop to be used instead of a NativeLoop built from agent.LLMs")
	}
	output, ok := result.(map[string]any)
	if !ok || output["url"] != "https://example.com/pr/2" {
